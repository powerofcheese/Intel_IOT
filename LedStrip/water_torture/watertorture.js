...
water_torture.prototype.droplet.prototype._draw = function () {
	...
	if (this.state == this.falling || this.state == this.bouncing)
	{
		...
		ledstrip.setLed(position8,1,this.parent.buffer[position8][0],this.parent.buffer[position8][1],this.parent.buffer[position8][2]);
		
		if (remainder)
		{
			this.add_clipped_to(this.parent.buffer[position8+1], this.scale(this.color, remainder));
			ledstrip.setLed(position8+1,1,this.parent.buffer[position8][0],this.parent.buffer[position8][1],this.parent.buffer[position8][2]);
		}

		if (this.state == this.bouncing)
		{
			this.add_clipped_to(this.parent.buffer[96 - 1], this.color);
			ledstrip.setLed(95,1,this.parent.buffer[95][0],this.parent.buffer[95][1],this.parent.buffer[95][2]);
		}
	}
	else if (this.state == this.swelling)
	{
		this.add_clipped_to(this.parent.buffer[0], this.scale(this.color, this.position));
		ledstrip.setLed(0,1,this.parent.buffer[0][0],this.parent.buffer[0][1],this.parent.buffer[0][2]);
	}
